...
export default function ProfileStatus({ user }: Props) {
  ...
  return (
    <div className='flex flex-row justify-center items-center gap-4 md:min-w-64 h-full'>
      <Image src={`${img}`} width={50} height={50} alt='avatar' className="rounded-xl w-[30px] h-[30px] md:w-[50px] md:h-[50px]" />
      <div>
        <h2 className="text-sm font-bold whitespace-nowrap">{nickname}</h2>
        <span className="text-[12px] md:text-sm text-[#8884d8]">{status}</span>
      </div>
